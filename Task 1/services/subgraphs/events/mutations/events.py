...
async def createEvent(details: InputEventDetails, info: Info) -> EventType:
    ...
    event_instance.status.last_updated_by = "8899898"
    ...
    # event_instance.code = f"EVENT-{random.randint(1000, 999999)}"
    unique_suffix = str(datetime.now().microsecond)
    event_instance.code = f"EVT-{details.clubid}-{unique_suffix}"
